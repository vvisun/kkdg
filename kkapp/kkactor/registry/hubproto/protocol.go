...
type (
	...
	RegisterActorReq struct {
		...
		ActorID  actortrans.ActorRef     `json:"actorID"`  // actor透明ID，用于注册或更新。
		NodeInfo *kkdiscovery.MemberInfo `json:"nodeInfo"` // actor所在节点信息。
	}
	RegisterActorResp struct {
		ReqID     uint64              `json:"reqID"`     // 请求ID，用于请求与响应的匹配。
		OpCode    int                 `json:"opCode"`    // 操作码，1表示注册，2表示注销。
		ActorID   actortrans.ActorRef `json:"actorID"`   // RegisterActorRequest中的actorID。
	...
	FindActorReq struct {
		ReqID   uint64              `json:"reqID"`   // 请求ID，用于请求与响应的匹配。
		ActorID actortrans.ActorRef `json:"actorID"` // 与 kkactor.LucencyID 语义一致（NodeID + ActorKey）。
	}
	FindActorResp struct {
		ReqID     uint64                  `json:"reqID"`     // 请求ID，用于请求与响应的匹配。
		ActorID   actortrans.ActorRef     `json:"actorID"`   // 找到的actorID。
	...
	GetAllActorsOfNodeResp struct {
		...
		Actors    []*actortrans.ActorRef  `json:"actors"`    // 节点上的所有actor列表。
		ErrorInfo *ErrorResp              `json:"errorInfo"` // 错误信息，如果请求处理失败，则返回错误信息。
	}
)
