...
func TestCombineActorKeys(t *testing.T) {
	tests := []struct {
		name    string
		keys    []string
		want    string
		...
		{"single", []string{"game"}, "game", nil},
		{"multi", []string{"game", "player"}, "game_player", nil},
		{"three", []string{"gate", "router", "session"}, "gate_router_session", nil},
		{"empty", []string{}, "", kkerrors.ErrActorInvalidActorKeys},
		{"nil", nil, "", kkerrors.ErrActorInvalidActorKeys},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				t.Errorf("CombineActorKeys() err = %v", err)
				return
			}
			if got != tt.want {
				t.Errorf("CombineActorKeys() = %q, want %q", got, tt.want)
			}
		})
	}
}

func TestCombineNodeAndActorKey(t *testing.T) {
	tests := []struct {
		name    string
		nodeId  string
		keys    []string
		...
		{"single_key", "game1", []string{"game_main"}, "game1/game_main", nil},
		{"multi_key", "gate1", []string{"gate", "router"}, "gate1/gate_router", nil},
		{"empty_keys", "game1", []string{}, "", kkerrors.ErrActorInvalidActorKeys},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				t.Errorf("CombineNodeAndActorKey() err = %v", err)
				return
			}
			if got != tt.want {
				t.Errorf("CombineNodeAndActorKey() = %q, want %q", got, tt.want)
			}
		})
	}
}

func TestGetActorName_GetActorId(t *testing.T) {
	tests := []struct {
		name     string
		nodeId   string
		actorKey string
		...
		{"with_node", "game1", "game_player", "game1/game_player"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			id, err := NewLucencyActorID(tt.nodeId, tt.actorKey)
			...
			name, err := GetActorName(id)
			if err != nil {
				t.Fatalf("GetActorName: %v", err)
			}
			if name != tt.wantName {
				t.Errorf("GetActorName() = %q, want %q", name, tt.wantName)
			}
			// round-trip
			id2, err := GetActorId(name)
			if err != nil {
				t.Fatalf("GetActorId: %v", err)
			}
			if id2.NodeID() != id.NodeID() || id2.ActorKey() != id.ActorKey() {
				t.Errorf("GetActorId round-trip: got %+v, want %+v", id2, id)
			}
		})
	}
}

func TestGetActorId_FromName(t *testing.T) {
	tests := []struct {
		name      string
		actorName string
		wantNode  string
		...
		{"local_only_key", "game_main", "", "game_main", false},
		{"node_and_key", "game1/game_player", "game1", "game_player", false},
		{"invalid_slash_in_key", "game1/game/player", "", "", true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			id, err := GetActorId(tt.actorName)
			if tt.wantErr {
				if err == nil {
					t.Errorf("GetActorId(%q) expected error", tt.actorName)
				}
				return
			}
			if err != nil {
				t.Errorf("GetActorId(%q) err = %v", tt.actorName, err)
				return
			}
			if id.NodeID() != tt.wantNode || id.ActorKey() != tt.wantKey {
				t.Errorf("GetActorId(%q) = node=%q key=%q, want node=%q key=%q",
					tt.actorName, id.NodeID(), id.ActorKey(), tt.wantNode, tt.wantKey)
			}
		})
	}
}
