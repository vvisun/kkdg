...
func (c *gnetClientConn) sendBufferDrop(buffer *kkbuffer.ByteBuffer) error {
	c.closeMu.Lock()
	if c.closing.Load() {
		c.closeMu.Unlock()
		kkbuffer.Put(buffer)
...
func (c *gnetClientConn) sendBufferRetry(buffer *kkbuffer.ByteBuffer) error {
...
func (c *gnetClientConn) startDrain() {
	c.closeMu.Lock()
...
func (c *gnetClientConn) releaseDrainedBatch(n int) {
	for i := 0; i < n; i++ {
		bb := c.drainBatch[i]
		c.drainBatch[i] = nil
		if bb != nil {
...
func (c *gnetClientConn) handleWriteError(err error) {
...
func (c *gnetClientConn) dropQueuedBuffers() {
	c.closeMu.Lock()
	c.draining = false
	for {
		bb := c.sendQueue.Pop()
