...
// Server represents a TLS WebSocket server based on gobwas/ws.
type Server struct {
	addr    string
	path    string
...
// NewServer creates a new TLS WebSocket server.
...
// Start begins listening and accepting TLS WebSocket connections.
func (s *Server) Start() error {
	...
	if s.opts.TLSConfig == nil {
		...
		return errors.New("kkwstls requires TLSConfig")
	}
	...
	go func() {
		s.opts.Logger.Infof("kkwstls server listen on %s%s", s.addr, s.path)
		close(s.booted)
		err := s.acceptTLS(ln)
		s.done <- err
	}()
...
func (s *Server) handleTLSConn(conn net.Conn) {
	...
	s.stats.OnConnect()
	if s.handler != nil {
		kknet.SafeHandlerCall(s.opts.Logger, &s.stats, "kkwstls OnConnect", func() {
...
func (s *Server) closeTLSConn(wc *netWSConn, err error) {
	...
	if s.handler != nil {
		kknet.SafeHandlerCall(s.opts.Logger, &s.stats, "kkwstls OnClose", func() {
			s.handler.OnClose(wc, err)
		})
	}
}

func sendCloseOnError(wc *netWSConn, err error) {
...
func (s *Server) Stop() error {
	...
	case <-done:
	case <-ctx.Done():
		s.opts.Logger.Warnf("kkwstls server shutdown timeout after %v", timeout)
	}
...
func (s *Server) dispatch(c kknet.IConn, data buffers.IBuffer) {
	...
	if s.pool == nil {
		...
		kknet.SafeHandlerCall(s.opts.Logger, &s.stats, "kkwstls OnMessage", func() {
	...
	if err := s.pool.Submit(func() {
		...
		kkbuffer.Put(data)
		s.stats.AddError()
		s.opts.Logger.Errorf("kkwstls submit task error: %v", err)
	}
}
