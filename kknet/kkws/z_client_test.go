...
func TestClient_NewClient(t *testing.T) {
	...
	// Conn() may return (nil, *wsConn) interface value before Connect; SendBuffer when not connected is tested in TestClient_SendBuffer_NotConnected
}
...
func TestClient_Connect_Close(t *testing.T) {
	...
	// double Connect is no-op (returns nil)
	if err := client.Connect(); err != nil {
		t.Fatalf("second Connect: %v", err)
	}
	// second Close when not connected
	_ = client.Close()
...
func TestClient_SendBuffer_NotConnected(t *testing.T) {
	opts := kknet.ApplyOptions(kknet.WithRawHandler(&noopRawHandler{}))
	client := NewClient("ws://127.0.0.1:9999/ws", nil, opts)
